...
$.keyboard = function(el, options){
	...
	base.keySelect = $('<select />')
		.attr({ 'role': 'select', 'aria-disabled': 'false' })
		.addClass('ui-keyboard-select ui-state-default ui-corner-all');
			
	base.addLayout = function (){
	    layoutSelect = $('<select />')
		.attr({ 'role': 'select', 'aria-disabled': 'false' })
		.attr('id', 'layout')
		.addClass('ui-keyboard-select ui-state-default ui-corner-all');
	    layoutSelect.append('<option value="albanian" class="albanian-qwerty">Albanian (qwerty)</option>');
	    layoutSelect.append('<option value="albanian" class="albanian-qwertz">Albanian (qwertz)</option>');
	    layoutSelect.append('<option value="arabic" class="arabic-azerty">Arabic (azerty)</option>');
	    layoutSelect.append('<option value="arabic" class="arabic-qwerty-1">Arabic (qwerty-1)</option>');
	    layoutSelect.append('<option value="arabic" class="arabic-qwerty-2">Arabic (qwerty-2)</option>');
	    layoutSelect.append('<option value="arabic" class="arabic-qwerty-3">Arabic (qwerty-3)</option>');
	    layoutSelect.append('<option value="arabic" class="arabic-qwerty-4">Arabic (qwerty-4)</option>');
	    layoutSelect.append('<option value="belarusian" class="belarusian-qwerty">Belarusian (Belarus)</option>');
	    layoutSelect.append('<option value="bengali" class="bengali-qwerty-1">Bengali (qwerty-1)</option>');
	    layoutSelect.append('<option value="bengali" class="bengali-qwerty-2">Bengali (qwerty-2)</option>');
	    layoutSelect.append('<option value="bulgarian" class="bulgarian-qwerty">Bulgarian (qwerty)</option>');
	    layoutSelect.append('<option value="catalan" class="catalan-qwerty">Catalan (qwerty)</option>');
	    layoutSelect.append('<option value="danish" class="danish-qwerty">Danish (qwerty)</option>');
	    layoutSelect.append('<option value="german" class="german-qwertz-1">German (qwertz-1)</option>');
	    layoutSelect.append('<option value="german" class="german-qwertz-2">German (qwertz-2)</option>');
	    layoutSelect.append('<option value="malayalam" class="malayalam-inscript">Malayalam Inscript</option>');
	    layoutSelect.append('<option value="russian" class="russian-qwerty">Russian (qwerty)</option>');
	    layoutSelect.append('<option value="swedish" class="swedish-qwerty">Swedish (qwerty)</option>');
	    layoutSelect.append('<option value="swedish" class="swedish-dvorak">Swedish (dvorak)</option>');
	    layoutSelect.change(function(){
		file = $(this).val();
		opt = $(this).find('option:selected');
		layoutName = opt.attr('class');
		base.$preview[file.match('arabic') ? 'addClass' : 'removeClass']('rtl');
		if (!$('script[src*=' + file + ']').length) {
			$.getScript('layouts/' + file + '.js', function() {
				if (base.$preview.data('keyboard')) { 
					base.destroy();
				}
				base.$preview.keyboard({layout: layoutName ,
				    usePreview: false,
				    alwaysOpen: false  
				});
			});
	    ...
	    return layoutSelect.appendTo(base.temp[0]);
	};
	// Add key function
	...
	$.keyboard.defaultOptions = {
		...
		display : {
			...
			'layouts': 'Select Keyboard Layout',
